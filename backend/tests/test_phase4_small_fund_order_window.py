...
from app.api.v1.exchange_accounts import record_small_fund_order_window_approval
...
from app.schemas.exchange_account import Phase4SmallFundOrderWindowApprovalRequest
...
def _user(role: UserRole = UserRole.SUPER_ADMIN) -> User:
    return User(
        email="phase4-window@example.com",
        username="phase4_window_operator",
    ...
    *,
    ...
    audit = AuditLog(
        ...
        payload={
            ...
            "acknowledgement": "ACKNOWLEDGE_SMALL_FUND_REVIEW_ONLY",
...
def _ready_account(db_session: Session) -> tuple[User, ExchangeAccount, AuditLog, AuditLog]:
    ...
    review_audit = _add_phase4_review_audit(
        db_session,
        owner=owner,
        account=account,
        read_only_audit=read_only_audit,
    )
    return owner, account, read_only_audit, review_audit


def test_phase4_small_fund_order_window_records_audit_without_authorizing_orders(
    db_session: Session,
) -> None:
    owner, account, read_only_audit, review_audit = _ready_account(db_session)
    ...
    assert approval.read_only_audit_log_id == read_only_audit.id
    assert approval.review_audit_log_id == review_audit.id
    assert approval.order_submission_authorized is False
    assert approval.trading_flags_changed is False

    audit = db_session.scalar(
        select(AuditLog).where(
            AuditLog.action == "real.small_fund.order_window.approval_recorded"
        )
    )
...
def test_phase4_small_fund_order_window_api_records_audit(
    db_session: Session,
) -> None:
    owner, account, _, _ = _ready_account(db_session)

    response = record_small_fund_order_window_approval(
        account.id,
    ...
    assert db_session.scalar(
        ...
        .where(AuditLog.action == "real.small_fund.order_window.approval_recorded")
    ) == 1


def test_phase4_small_fund_order_window_requires_super_admin(
    db_session: Session,
) -> None:
    ...
    with pytest.raises(Phase4SmallFundOrderWindowBlockedError) as error:
        record_phase4_small_fund_order_window_approval(
            db_session,
            user_id=owner.id,
            user_role=owner.role,
...
def test_phase4_small_fund_order_window_requires_review_audit(
    db_session: Session,
) -> None:
    owner = _user()
    db_session.add(owner)
    ...
    assert "Phase 4 small-fund review audit is required" in error.value.reasons


def test_phase4_small_fund_order_window_blocks_notional_above_review_cap(
    db_session: Session,
) -> None:
    owner, account, _, _ = _ready_account(db_session)
    ...
    with pytest.raises(Phase4SmallFundOrderWindowBlockedError) as error:
        record_phase4_small_fund_order_window_approval(
            ...
            symbol="BTC-USDT",
            side=OrderSide.BUY,
            max_quantity=Decimal("0.002"),
            limit_price=Decimal("20000"),
            max_notional=Decimal("40"),
            duration_minutes=5,
            acknowledgement=PHASE4_SMALL_FUND_ORDER_WINDOW_ACK,
        )

    assert "order window max_notional exceeds the reviewed small-fund cap" in error.value.reasons


def test_phase4_small_fund_order_window_blocks_trading_enabled_state(
    db_session: Session,
) -> None:
    owner = _user()
    db_session.add(owner)
