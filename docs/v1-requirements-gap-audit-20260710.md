...
| 全局 Kill Switch | 已实现 | 持久化全局状态、超级管理员 API/UI、审计与所有新订单入口 fail-closed 检查已存在。 |
...
| Order State Machine | 已有统一状态转换校验和只追加转换历史；受审批 TESTNET 提交会先持久化执行记录并经过数量、币种和金额风控，成功响应进入 `ACCEPTED`，失败响应进入 `FAILED`。Binance、Bybit、OKX 订单事件已有标准化和租户隔离处理器，可去重并拒绝乱序或非法转换，但尚未接入持续消费进程。 | 接入持续 WebSocket 消费、REST 补数、超时与对账处理。 |
...
4. 正式行情图已有公开 REST 历史数据、WebSocket 实时更新和多来源切换；订单簿、账户权益、活动订单和成交历史尚未由完整交易所实时数据流支持。
